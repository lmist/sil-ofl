...
export function FontRow(input: FontRowInput) {
  ...
  return (
    <button
      {...row.rootProps}
      className={cn(
        "group flex h-full min-w-0 w-full flex-col justify-center gap-3 border-b border-border px-0 py-6 text-left",
    ...
    >
      ...
      <span className="flex min-w-0 max-w-full flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <span
          {...row.nameProps}
          className={cn(
            "min-w-0 max-w-full break-words text-[0.8125rem] tracking-tight text-foreground",
        ...
        <span
          {...row.metaProps}
          className="min-w-0 max-w-full break-words text-[0.75rem] tabular-nums text-muted-foreground sm:shrink-0"
        >
          {row.meta}
        </span>
      </span>
